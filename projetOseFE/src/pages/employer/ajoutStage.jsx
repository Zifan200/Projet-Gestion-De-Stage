...
export default function AjoutStage() {
  ...
  const validate = () => {
    ...
    if (!formData.description.trim())
      tempErrors.description = "Une description est obligatoire.";
    if (!formData.targetedProgramme.trim())
      tempErrors.targetedProgramme = "Un programme est obligatoire.";
    if (!formData.employerEmail.trim())
      tempErrors.employerEmail = "Courriel est obligatoire.";
    else if (!/\S+@\S+\.\S+/.test(formData.employerEmail))
      tempErrors.employerEmail = "Courriel est invalide.";
    if (!formData.expirationDate)
      tempErrors.expirationDate = "Une date limite est obligatoire.";
  ...
  return (
    ...
    >
      <h1 className="text-4xl mt-5 font-bold">
        Une nouvelle opportunité, à quelques clics.
      </h1>
      <p className="text-2xl my-5 font-bold">
        Publiez vos offres de stage à divers étudiants dès maintenant.
      </p>
      {successMessage && (
      ...
      {errorMessage && (
        <p className="text-lg text-center font-bold text-red-700">
          {errorMessage} <br></br>
          Assurez-vous d'entrer un courriel déjà enregistré sur la plateforme !
        </p>
      )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={"flex flex-col mb-7"}>
          ...
          <input
            className="border-1 shadow uppercase rounded-sm mb-1 text-gray-700 px-2 py-2"
            type="date"
            name="expirationDate"
        ...
        <button
          className="w-sm cursor-pointer p-4 bg-black rounded-xl  text-white text-xl
                                   shadow duration-500 hover:border-1 hover:font-semibold
                                   hover:text-2xl mb-10 transition-all"
          type="submit"
        >
          Soumettre l'offre
        </button>
