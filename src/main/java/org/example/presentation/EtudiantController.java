...
public class EtudiantController {
    ...
    public ResponseEntity<EtudiantDTO> inscription(
        ...
        EtudiantDTO savedEtudiant = studentService.inscriptionEtudiant(
            etudiantDTO
        );
        ...
        emailEtudiant.setBody(
            "<p>Bonjour <strong>" +
                savedEtudiant.getLastName() +
                " " +
                savedEtudiant.getLastName() +
                "</strong>,</p>" +
                "<p>Votre inscription au programme <strong>" +
                savedEtudiant.getProgram() +
    ...
    public ResponseEntity<
        InternshipApplicationResponseDTO
    > getInternshipApplicationForStudentById(
        HttpServletRequest request,
        ...
        return ResponseEntity.ok(
            internshipApplicationService.getApplicationByStudentAndId(email, id)
        );
    }
    ...
    > getAllInternshipApplicationsForStudent(HttpServletRequest request) {
        String email = userAppService
            .getMe(JwtTokenUtils.getTokenFromRequest(request))
            .getEmail();
        return ResponseEntity.ok(
            internshipApplicationService.getAllApplicationsFromStudent(email)
        );
    }
    ...
    public ResponseEntity<
        List<InternshipApplicationResponseDTO>
    > getAllInternshipApplicationsForStudentByStatus(
        HttpServletRequest request,
    ...
    public ResponseEntity<
        List<InternshipApplicationResponseDTO>
    > getAcceptedApplicationsForStudent(HttpServletRequest request) {
        String studentEmail = userAppService
            .getMe(JwtTokenUtils.getTokenFromRequest(request))
            .getEmail();

        List<InternshipApplicationResponseDTO> acceptedApplications =
            internshipApplicationService.getAcceptedApplicationsForStudent(
                studentEmail
            );
    ...
    public ResponseEntity<ConvocationDTO> updateConvocationStatus(
        @RequestBody ConvocationDTO convocationDto
    ) {
        return ResponseEntity.status(HttpStatus.OK).body(
            convocationService.updateConvocationStatus(
    ...
    public ResponseEntity<List<ConvocationDTO>> getConvocationsForStudent(
        ...
        String studentEmail = userAppService
            ...
            .getEmail();
        return ResponseEntity.ok(
            convocationService.getAllConvocationsForStudent(studentEmail)
        );
    }

    @GetMapping("/my-teacher")
    public ResponseEntity<TeacherDTO> getMyTeacher(HttpServletRequest request) {
        String studentEmail = userAppService
            .getMe(JwtTokenUtils.getTokenFromRequest(request))
            .getEmail();
        TeacherDTO teacher = studentService.getMyTeacher(studentEmail);
        if (teacher == null) {
