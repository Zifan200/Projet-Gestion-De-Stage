...
public class InternshipOfferServiceTest {
    ...
    private InternshipOffer buildInternshipOffer(
        Employer employer,
        LocalDate offerPublishDateTime
    ) {
    ...
    private InternshipOfferDto buildInternshipOfferDto(
        String email,
        LocalDate offerPublishDateTime
    ) {
    ...
    void createInternshipOffer_shouldSaveInternshipOffer() {
        ...
        InternshipOffer createdOffer = buildInternshipOffer(
            employer,
            offerPublishDateTime
        );
        ...
        when(
            internshipOfferRepository.save(any(InternshipOffer.class))
        ).thenReturn(createdOffer);

        //Act
        InternshipOfferDto internshipOfferDto = buildInternshipOfferDto(
        ...
        assertThat(savedOffer)
            .extracting(
                InternshipOfferResponseDto::getTitle,
                InternshipOfferResponseDto::getPublishedDate
            )
            .containsExactly(
                "recherche Scratch Developer",
                offerPublishDateTime
            );

        verify(internshipOfferRepository).save(any(InternshipOffer.class));
        verify(eventPublisher).publishEvent(
            any(EmployerCreatedInternshipOfferEvent.class)
        );
    ...
    void createInternshipOffer_shouldHaveReferenceToEmployer() {
        ...
        InternshipOfferResponseDto savedOffer =
            internshipOfferService.saveInternshipOffer(
                internshipOfferDto.getEmployerEmail(),
                internshipOfferDto
            );
        ...
        assertTrue(
            savedOffer.getEmployerEmail().equalsIgnoreCase("test@google.com")
        );
        assertSame(savedOffer.getPublishedDate(), offerPublishDateTime);
        assertNotNull(
            employerRepository.findByCredentialsEmail("test@google.com")
        );
    ...
    void getAllOffers_shouldReturnListOfOffers() {
        // Arrange
        Employer employer = buildEmployer();
        InternshipOffer offer1 = buildInternshipOffer(
            employer,
            LocalDate.now()
        );
        InternshipOffer offer2 = buildInternshipOffer(
            employer,
            LocalDate.now().plusDays(1)
        );
        offer2.setTitle("Frontend Developer");

        when(internshipOfferRepository.findAll()).thenReturn(
        ...
        assertThat(offers.get(0).getTitle()).isEqualTo(
            "recherche Scratch Developer"
        );
    ...
    void getOffersByProgramme_shouldReturnFilteredOffers() {
        ...
        assertThat(filteredOffers.getFirst().getTitle()).isEqualTo(
            "recherche Scratch Developer"
        );
        assertThat(filteredOffers.getFirst().getEnterpriseName()).isEqualTo(
            employer.getEnterpriseName()
        );
    }
    ...
    void getAllTargetedProgrammes_shouldReturnDistinctSortedList() {
        ...
        offer2.setTargetedProgramme("Science de la nature");
        InternshipOffer offer3 = buildInternshipOffer(
            employer,
            LocalDate.now()
        );
        ...
        assertThat(programmes).containsExactly(
            "Informatique",
    ...
    void getOffersByProgramme_whenNoMatchingOffers_shouldReturnEmptyList() {
        ...
        // Act
        List<InternshipOfferListDto> filteredOffers =
            internshipOfferService.getOffersByProgramme("Science de la nature");
    ...
    void updateOfferStatus_shouldUpdateStatusSuccessfully() {
        ...
        when(internshipOfferRepository.findById(1L)).thenReturn(
            Optional.of(offer)
        );
        when(
            internshipOfferRepository.save(any(InternshipOffer.class))
    ...
    void updateOfferStatus_shouldThrowWhenOfferIdNotFound() {
        // Arrange
        when(internshipOfferRepository.findById(99L)).thenReturn(
            Optional.empty()
        );
    ...
    void getAcceptedOffers_shouldReturnOnlyAcceptedOffers() {
        ...
        offer1.setVisibleToStudents(true);
        ...
        offer2.setVisibleToStudents(true);
        ...
        offer3.setVisibleToStudents(true);
        ...
        // Act
        List<InternshipOfferListDto> acceptedOffers =
            internshipOfferService.getAcceptedOffers();

        // Assert
        assertThat(acceptedOffers).hasSize(2);
        assertThat(acceptedOffers)
            .extracting("title")
    ...
    void generateInternshipOfferPdfTest_Success() throws IOException {
        ...
        when(internshipOfferRepository.findInternshipOffersById(1L)).thenReturn(
            Optional.of(offer)
        );

        when(
            employerRepository.findByCredentialsEmail(employer.getEmail())
        ).thenReturn(Optional.of(employer));
        ...
        assertTrue(
            pdfBytes.length > 100,
            "Le PDF généré doit contenir des octets"
        );
        verify(internshipOfferRepository, times(1)).findInternshipOffersById(
            1L
        );
    }
    ...
    void generateInternshipOfferPdfTest_Failure() {
        // Arrange
        when(internshipOfferRepository.findInternshipOffersById(2L)).thenReturn(
            Optional.empty()
        );
    ...
    void getAllOffersSummaryFromEmployer_shouldReturnOffersOfEmployer() {
        ...
        // Act
        List<InternshipOfferListDto> offers =
            internshipOfferService.getAllOffersSummaryFromEmployer(
                employer.getEmail()
            );
    ...
    void getAllOffersSummaryFromEmployer_whenEmployerNotFound_shouldThrow() {
        ...
        when(employerRepository.findByCredentialsEmail(email)).thenReturn(
            Optional.empty()
        );
    ...
    @Test
    void updateOfferVisibility_shouldUpdateVisibilityToTrue() {
        // Arrange
        Employer employer = buildEmployer();
        InternshipOffer offer = buildInternshipOffer(employer, LocalDate.now());
        offer.setId(1L);
        offer.setVisibleToStudents(false);
        ...
        InternshipOfferResponseDto response =
            internshipOfferService.updateOfferVisibility(1L, true);

        // Assert
        assertNotNull(response);
        assertTrue(response.isVisibleToStudents());
        verify(internshipOfferRepository).findById(1L);
        verify(internshipOfferRepository).save(offer);
    }

    @Test
    void updateOfferVisibility_shouldUpdateVisibilityToFalse() {
        // Arrange
        Employer employer = buildEmployer();
        InternshipOffer offer = buildInternshipOffer(employer, LocalDate.now());
        offer.setId(1L);
        ...
        // Act
        InternshipOfferResponseDto response =
            internshipOfferService.updateOfferVisibility(1L, false);

        // Assert
        assertNotNull(response);
        assertFalse(response.isVisibleToStudents());
        verify(internshipOfferRepository).findById(1L);
        verify(internshipOfferRepository).save(offer);
    }

    @Test
    void updateOfferVisibility_shouldThrowWhenOfferNotFound() {
        // Arrange
        when(internshipOfferRepository.findById(999L)).thenReturn(
            Optional.empty()
        );

        // Act & Assert
        assertThatThrownBy(() ->
            internshipOfferService.updateOfferVisibility(999L, true)
        )
            .isInstanceOf(InvalidInternShipOffer.class)
            .hasMessageContaining("Offer not found with id: 999");

        verify(internshipOfferRepository).findById(999L);
        verify(internshipOfferRepository, never()).save(any());
    }

    @Test
    void getAcceptedOffers_shouldFilterByVisibility() {
        // Arrange
        Employer employer = buildEmployer();
        ...
        offer2.setId(2L);
        offer2.setStatus(ApprovalStatus.ACCEPTED);
        offer2.setVisibleToStudents(false);
    ...
    @Test
    void getAcceptedOffersByProgramme_shouldFilterByVisibility() {
        // Arrange
        Employer employer = buildEmployer();
        ...
        offer2.setId(2L);
        offer2.setStatus(ApprovalStatus.ACCEPTED);
        offer2.setTargetedProgramme("Informatique");
        offer2.setVisibleToStudents(false);
        ...
        offer3.setId(3L);
        offer3.setStatus(ApprovalStatus.ACCEPTED);
        offer3.setTargetedProgramme("Informatique");
        offer3.setVisibleToStudents(true);

        InternshipOffer offer4 = buildInternshipOffer(
            employer,
            LocalDate.now()
        );
        offer4.setId(4L);
        offer4.setStatus(ApprovalStatus.ACCEPTED);
        offer4.setTargetedProgramme("Science");
        offer4.setVisibleToStudents(true);

        when(internshipOfferRepository.findAll()).thenReturn(
            List.of(offer1, offer2, offer3, offer4)
        );

        // Act
        List<InternshipOfferListDto> filteredOffers =
            internshipOfferService.getAcceptedOffersByProgramme("Informatique");

        // Assert
        assertThat(filteredOffers).hasSize(2);
        assertThat(filteredOffers)
            .extracting("id")
            .containsExactlyInAnyOrder(1L, 3L);
    }
}
